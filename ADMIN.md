...
5. [保护类（GP / WG / Plot）](#5-保护类griefprevention--worldguard--plotsquared)
6. [FastAsyncWorldEdit (FAWE)](#6-fastasyncworldedit-fawe)
7. [CoreProtect 行为日志](#7-coreprotect-行为日志)
8. [BlueMap 网页地图](#8-bluemap-网页地图)
9. [Chunky 区块预生成](#9-chunky-区块预生成)
10. [DeluxeMenus 菜单](#10-deluxemenus-菜单)
11. [性能监控（spark）](#11-性能监控spark)
12. [PlugManX 插件热重载](#12-plugmanx-插件热重载)
13. [日常运维节奏](#13-日常运维节奏)
14. [应急处置](#14-应急处置)
...
推荐保留的世界：`world`(主世界) / `world_nether`(下界) / `world_the_end`(末地) / `world_resource`(资源世界，定期重置) / `plotworld`(地皮世界)。
...
## 5. 保护类（GriefPrevention / WorldGuard / PlotSquared）

三个保护插件分工：
- **GriefPrevention**：玩家自助圈地（金铲子）
- **WorldGuard**：管理员定义全局规则区域（PVP / 安全区 / 禁飞区）
- **PlotSquared**：地皮世界（固定大小的整齐建造区）
...
### 5.3 PlotSquared（地皮世界）

| 命令 | 用途 |
|------|------|
| `/plot setup` | **交互式创建一个地皮世界**。完成后 `/mv import 世界名 NORMAL -g PlotSquared` |
| `/plot area create` | 在已有世界中划出地皮区域 |
| `/plot reset` / `/plot clear` | 重置脚下地皮为空白 |
| `/plot setowner <玩家>` | 修改脚下地皮所有者 |
| `/plot purge <玩家>` | 清理某玩家的地皮（需二次确认） |
| `/plot debugexec expired-result-purge` | 清理过期地皮 |
| `/plot flag set <标志> <值>` | 设置标志：`music` `weather` `time` `pve true` `fly true` |

---

## 6. FastAsyncWorldEdit (FAWE)
...
- 清理过期地皮：`/plot debugexec expired-result-purge`。
